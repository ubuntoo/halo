...
class PDODb implements IDaoHelper
{
     ...
     * @return void
     */
    public function __construct($args = array(), $connect = true)
    {	  	
    	if (empty(self::$connArgs) && $args) {
    		self::$connArgs = $args;
    	}
    	if (self::$pdo || !$connect) return;
    	// 连接数据库
    	$dsn = "mysql:";
        if (isset($args['host']))        $dsn .= "host=" . $args['host'] . ";";
        if (isset($args['port']))        $dsn .= "port=" . $args['port'] . ";";
        if (isset($args["dbname"]))      $dsn .= "dbname=" . $args["dbname"] . ";";
        if (isset($args["unix_socket"])) $dsn .= "unix_socket=" . $args["unix_socket"] . ";";
        try {
        	self::$pdo = new PDO(
            	$dsn,
                ...
                array(
                	PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'UTF8';",
                 	...
                 	PDO::ATTR_PERSISTENT => isset($args['pconnect']) && $args['pconnect'] ? true : false,
               	)
           	);
     ...
     * @param string $fields 字段
     * @param string $where 条件
     * @param unknown_type $intoObj 绑定对象
     */
